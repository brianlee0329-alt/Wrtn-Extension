...
!(async function () {
  ...
  const {
    ...
    VertexAIBackend,
    // @ts-ignore
  } = await import("https://www.gstatic.com/firebasejs/12.8.0/firebase-ai.js");
  ...
  class ReconstructableResponse {
    ...
    static fromJson(json) {
      return new ReconstructableResponse(json.chatId, json.name, json.response, json.input, json.output);
    }
    ...
    async doRequest(option) {
      try {
        if (!settings.config.geminiKey) {
          return new LLMError(-1, "Gemini API 키가 등록되지 않았습니다.", false);
        }
        ...
        const request = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${option.modelId}:generateContent?key=${settings.config.geminiKey}`, {
          method: "POST",
        ...
        if (Math.floor(request.status / 100) !== 2) {
          if (request.status === 403) {
            return new LLMError(request.status, "Gemini API 키가 유효하지 않거나 권한이 부족합니다.", false);
          }
          if (request.status === 429) {
            return new LLMError(request.status, "Gemini API의 레이트리밋에 도달하여 요청에 실패하였습니다.", true);
          }
          if (request.status === 500) {
            return new LLMError(request.status, "Gemini API에서 서버 오류가 발생하였습니다.", true);
          }
        ...
        if (!candidates || candidates.length <= 0) {
          return new LLMError(1400, "LLM에서 응답 컨테이너를 반환하지 않았습니다.", true);
        }
        const parts = candidates[0].content?.parts;
        if (!parts || parts.length <= 0) {
          return new LLMError(1400, "LLM에서 응답 데이터를 반환하지 않았습니다.", true);
        }
        const message = parts[0].text;
        if (!message) {
          return new LLMError(1400, "LLM에서 빈 응답 메시지를 반환하였습니다.", true);
        }
        return new GenericLLMResponse(message, new TokenUsage(json.usageMetadata.promptTokenCount, json.usageMetadata.candidatesTokenCount));
      } catch (e) {
        console.error(e);
    ...
    async doRequest(option) {
      try {
        if (!settings.config.firebaseScript) {
          return new LLMError(-1, "Firebase API 스크립트가 등록되지 않았습니다.", false);
        }
        const extracted = parseVertexContent(settings.config.firebaseScript);
        if (!extracted) {
          return new LLMError(-1, "잘못된 Firebase API 스크립트가 입력되어 있습니다.", false);
        }
        let app = undefined;
        try {
          ...
          console.error(e);
          return new LLMError(-1, "Firebase API 오류: 잘못된 API 키 혹은 스크립트가 입력되었습니다.", false);
        }
        try {
          const ai = getAI(app, {
          ...
          return new GenericLLMResponse(text, new TokenUsage(response.usageMetadata.promptTokenCount, response.usageMetadata.candidatesTokenCount));
        } catch (error) {
          throw error;
        }
      } catch (e) {
  ...
  const settings = new LocaleStorageConfig(PLATFORM_SAVE_KEY, {
    ...
    promptPrefixMessage: "**OOC: 현재까지의 롤플레잉 진행상황 요약입니다. 이후 응답에 이 요약 내용을 참조하겠습니다.**",
    promptSuffixMessage: "",
    /** Optional paramers */
    /** @type {string | undefined} */
  ...
  function appendBurnerLog(message) {
    const logContainer = GenericUtil.refine(document.getElementById("chasm-ignt-log-container"));
    if (!logContainer) return;
    const time = new Date();
    const timeMessage = `[${time.getHours().toString().padStart(2, "0")}:${time.getMinutes().toString().padStart(2, "0")}:${time.getSeconds().toString().padStart(2, "0")}] `;
    logContainer.textContent = timeMessage + message + "\n" + logContainer.textContent;
  }
  ...
  function setupModal() {
    ...
    modal.addLicenseDisplay((appender) => {
      ...
      appender.addText("결정화 캐즘 이그나이터는 다음 프롬프트를 허가받아 탑재하고 있습니다 :");
      appender.addText("- 구조기억 프롬프트 (https://gall.dcinside.com/mini/wrtnw/90088)");
      appender.addText("- 신디사이저 프롬프트 (https://gall.dcinside.com/mini/wrtnw/106196)");
      appender.addText("이 외의 기타 프롬프트는 캐즘 버너 코드에서 암묵적으로 허가되었다고 판단되어 탑재되었습니다.");
    });
    modal
    ...
    modal.getOpened().withPreOpenHandler((modalItem) => {
      ...
      for (let i = 1; i <= currentTopCount; i++) {
        const text = sessionStorage.getItem("chasm-ignt-resp-" + chatId + "-" + i);
        if (text) {
          const reconstructed = ReconstructableResponse.fromJson(JSON.parse(text)).withCurrentSessionId("chasm-ignt-resp-" + chatId + "-" + i);
  ...
  function setupBurnerPage(panel) {
    const providerBox = panel.constructSelectBox("모델 제공자", "Google", "chasm-ignt-provider-Google", false);
    const modelBox = panel.constructSelectBox("사용 모델", "Gemini 2.5 Pro", "gemini-2.5-pro", false);
      ...
      providerBox.addOption(provider, `${provider}`, (id, node) => {
        ...
        for (const model of Object.keys(MODEL_MAPPINGS[provider])) {
          ...
          modelBox.addOption(item.display, `${item.name}`, (idModel, node) => {
            ...
            settings.save();
            if (item.warning) {
              const element = panel.__element.getElementsByClassName("chasm-ignt-warning");
              element[0].removeAttribute("chasm-ignt-hide");
              element[0].textContent = `⚠ ${item.warning}`;
            } else {
              panel.__element.getElementsByClassName("chasm-ignt-warning")[0].setAttribute("chasm-ignt-hide", "true");
            }
            return true;
          });
        }
        if (document.getElementById("chasm-ignt-warning")) {
          modelBox.setSelected(
            // @ts-ignore
            MODEL_MAPPINGS[provider][Object.keys(MODEL_MAPPINGS[provider])[0]].name,
          );
    ...
    const promptPreset = panel.constructSelectBox("프롬프트 프리셋", "커스텀", "chasm-ignt-prompt-index-0", true);
    let uniqueIndex = 0;
    for (const author of Object.keys(DEFAULT_PROMPTS)) {
      ...
      for (const preset of Object.keys(DEFAULT_PROMPTS[author])) {
        const node = promptPreset.addOption(preset, `chasm-ignt-prompt-index-${uniqueIndex++}`, (id, node) => {
          // @ts-ignore
          const selected = DEFAULT_PROMPTS[author][preset];
          if (selected) {
            const descriptionField = GenericUtil.refine(document.getElementById("chasm-ignt-description-field"));
          ...
          const descriptionField = document.getElementById("chasm-ignt-description-field");
          descriptionField?.parentElement?.parentElement?.parentElement?.removeAttribute("chasm-ignt-hide");

          if (settings.config.addPromptModifyBox) {
            GenericUtil.refine(document.getElementById("chasm-ignt-custom-prompt")).value =
              // @ts-ignore
              JSON.stringify(DEFAULT_PROMPTS[author][preset].prompt);
          } else {
            document.getElementById("chasm-ignt-custom-prompt")?.parentElement?.setAttribute("chasm-ignt-hide", "true");
          }
          settings.save();
    ...
    promptPreset.addOption("커스텀 프롬프트", "chasm-ignt-prompt-option-custom", (id, node) => {
      // @ts-ignore
      settings.config.lastSelected = ["$custom"];
      document.getElementById("chasm-ignt-custom-prompt")?.parentElement?.removeAttribute("chasm-ignt-hide");
      document.getElementById("chasm-ignt-description-field")?.parentElement?.parentElement?.parentElement?.setAttribute("chasm-ignt-hide", "true");
      settings.save();
    ...
    database.prompts
      ...
      .then((/** @type {string | any[]} */ arr) => {
        if (arr.length > 0) {
          ...
          for (let item of arr) {
            const element = promptPreset.addOption(item.name, `#LIBRARY_PRESET!:${item.name}`, (_) => {
              const descriptionField = GenericUtil.refine(document.getElementById("chasm-ignt-description-field"));
              descriptionField.textContent = item.description.length <= 0 ? "이 프롬프트에는 설명이 없습니다." : item.description;
              // @ts-ignore
              settings.config.lastSelected = ["$prompt-library", item.name];
              descriptionField?.parentElement?.parentElement?.parentElement?.removeAttribute("chasm-ignt-hide");
              if (settings.config.addPromptModifyBox) {
                GenericUtil.refine(document.getElementById("chasm-ignt-custom-prompt")).value = item.prompt;
              } else {
                document.getElementById("chasm-ignt-custom-prompt")?.parentElement?.setAttribute("chasm-ignt-hide", "true");
              }
              settings.save();
        ...
        if (settings.config.lastSelected) {
          ...
          if (settings.config.lastSelected[0] === "$prompt-library") {
            ...
            for (const element of promptPreset.node.getElementsByClassName("chasm-ignt-embedded-prompt")) {
              if (
    ...
    const customPromptBox = panel.constructTextAreaGrid("chasm-ignt-custom-prompt", "커스텀 프롬프트", {
      ...
      onInit: (node) => {
        if (settings.config.addPromptModifyBox) {
          node.oninput = () => {
            if (promptPreset.getSelected() !== "chasm-ignt-prompt-option-custom") {
              // @ts-ignore
    ...
    if (!settings.config.addPromptModifyBox) {
      GenericUtil.refine(customPromptBox.parentElement).setAttribute("chasm-ignt-hide", "true");
    }
    panel.addSwitchBox("chasm-ignt-auto-retry", "오류 발생시 자동 재시도", "활성화시, 서버에서 오류가 발생할 경우 자동으로 재시도합니다.\n체크 해제시 자동 재시도가 중단됩니다.", {
    ...
    const messageCountUnit = settings.config.useLegacyTurnLogic ? "턴" : "메시지";
    panel.addShortNumberBox("chasm-ignt-max-item", `요약에 불러올 ${messageCountUnit}`, `요약에 불러올 ${messageCountUnit}를 지정합니다. 기본 50이며, 0으로 설정시 모든 메시지를 불러옵니다. \n값이 클 수록 요청 시간이 길어지며 소모 비용 또한 늘어납니다.`, {
    ...
    panel.addSwitchBox("chasm-ignt-add-random-header", "전송에 무작위 헤더 추가", "활성화시, 전송될 메시지에 유저노트를 적용합니다. \n무작위 헤더는 50자의 프롬프트를 사용하지만, LLM의 캐시 무효화를 유도하여 응답 확률을 올립니다.", {
    ...
    panel.addSwitchBox("chasm-ignt-add-persona-data", "페르소나 데이터 첨부", "활성화시, LLM에 전송될 메시지에 페르소나 데이터 (대화 프로필)을 첨부합니다.", {
    ...
    panel.addSwitchBox("chasm-ignt-add-user-note", "유저노트 첨부", "활성화시, LLM에 전송될 메시지에 유저노트 데이터을 첨부합니다.", {
    ...
    panel.addBoxedInputGrid("chasm-ignt-ooc-user-message", "사용자 입력 메시지", "이그나이터로 생성된 메시지를 전송할 때, 고정 유저 메시지를 설정합니다.", {
    ...
    panel.addBoxedInputGrid("chasm-ignt-prompt-prefix", "요약 메시지 머릿말", "이그나이터로 생성된 메시지를 전송할 때, 맨 앞에 첨부될 메시지를 설정합니다.", {
    ...
    panel.addBoxedInputGrid("chasm-ignt-prompt-suffix", "요약 메시지 꼬릿말", "이그나이터로 생성된 메시지를 전송할 때, 맨 뒤에 첨부될 메시지를 설정합니다.", {
      ...
      .addButton("chasm-ignt-start-request", "시작하기", {
        ...
        onTrigger: (node) => {
          ...
          startProcess()
            .catch((err) => {
              console.error(err);
              appendBurnerLog("알 수 없는 오류가 발생하였습니다. 자세한 사항은 콘솔을 확인하세요. (" + err.message + ")");
            })
            .then(() => {
  ...
  async function startProcess() {
    ...
    const messages = await fetcher.fetch(settings.config.maxMessageRetreive * (settings.config.useLegacyTurnLogic ? 2 : 1));
    if (messages instanceof Error) {
      appendBurnerLog("메시지를 가져오는 중 오류가 발생하였습니다. 콘솔을 확인하세요.");
      console.error(messages);
      return false;
    }
    appendBurnerLog(`${settings.config.useLegacyTurnLogic ? Math.floor(messages.length / 2) : messages.length}개의 ${settings.config.useLegacyTurnLogic ? "턴을" : "메시지를"} 불러왔습니다.`);
    ...
    if (settings.config.includePersona) {
      ...
      // @ts-ignore
      messageStructure.chatLog = messages.map((it) => it.withPersona(persona.name));
      // @ts-ignore
    ...
    if (settings.config.includeUserNote) {
      ...
      if (userNote.length > 0) {
        ...
        appendBurnerLog("유저노트 데이터가 없습니다. 유저노트 첨부 없이 진행합니다.");
      }
    }
    ...
    while (true) {
      ...
      if (result instanceof LLMError) {
        appendBurnerLog("LLM 요청 실패: 오류 코드 " + result.code + ": " + result.message);
        if (!settings.config.useAutoRetry) {
          return false;
        }
        if (!result.isRecoverable) {
          appendBurnerLog("이 오류는 재시도 불가능한 오류입니다. 재시도를 중단합니다.");
          return false;
        }
        if (result.code === 429) {
          appendBurnerLog("API 요청이 레이트리밋에 도달하였습니다. 10초 후에 다시 시도합니다..");
        ...
        const reconstructed = new ReconstructableResponse(chatId, nextName, result.message, result.tokenUsage.input, result.tokenUsage.output).withCurrentSessionId("chasm-ignt-resp-" + chatId + "-" + nextId);
        sessionStorage.setItem("chasm-ignt-resp-" + chatId + "-" + nextId, JSON.stringify(reconstructed));
        const modal = ModalManager.getOrCreateManager("c2");
  ...
  function addMenuFromConstructables(modal, result) {
    modal
      ...
      .createSubMenu(result.name, (modal) => {
        modal.replaceContentPanel((panel) => {
          panel.addTextAreaGrid("chasm-ignt-result-panel", "요청 결과", {
            ...
            onInit: (node, _, suffix) => {
              ...
              node.oninput = (ev) => {
                GenericUtil.refine(document.getElementById("chasm-ignt-result-length-display")).textContent = `${GenericUtil.refine(node).value.length}자`;
              };
              if (suffix) {
                new ComponentAppender(suffix).addText(`${result.response.length}자`, {
                  initializer: (textNode) => {
            ...
            onChange: (_, text) => {
              if (settings.config.saveModifiedResult) {
                sessionStorage.setItem(result.sessionId ?? "", JSON.stringify(result.withResponse(text)));
              }
            },
          });
          panel.addText(`전체 ${result.total()} 토큰, 입력 ${result.input} 토큰, 출력 ${result.output} 토큰`);
          ...
          panel
            ...
            .addButton("chasm-ignt-show-html", "HTML 표시", {
              onTrigger: () => {
                ...
                topDivision.style.cssText = "display: flex; flex-direction: column; align-items: center; z-index: 99999 !important; position: absolute; top: 0; left: 0; width: 100%; height: 100%; background-color: white; ";
                const topBar = document.createElement("div");
                topBar.style.cssText = "display: flex; flex-direction: row; align-items: center; width: 100%; height: 32px; background-color: #333; border-bottom: 1px solid #555;";
                const comment = document.createElement("span");
                comment.style.cssText = "font-size: 0.8em; color: #eee; margin-left: 24px;";
                comment.textContent = allowJS ? `C2 Ignitor ${VERSION} HTML Preview (Full Mode)` : `C2 Ignitor ${VERSION} HTML Preview (Sandbox Mode)`;
                topBar.append(comment);
                const closer = document.createElement("div");
                closer.innerHTML = CLOSE_ICON_SVG;
                closer.style.cssText = "width: fit-content; margin-left: auto; height: 16px; width: 16px; cursor: pointer; margin-right: 16px;";
                ...
                newItem.style.cssText = "flex: 1 !important; width: 100% !important;";
                if (!allowJS) {
                  newItem.setAttribute("sandbox", "");
                }
                newItem.setAttribute("srcdoc", GenericUtil.refine(document.getElementById("chasm-ignt-result-panel")).value.replace('"', "&quot;"));
            ...
            .addButton("chasm-ignt-start-send", "전송하기", {
              onTrigger: (node) => {
                ...
                const statusTextNode = GenericUtil.refine(document.getElementById("chasm-ignt-status-text"));
                let messageToSend = GenericUtil.refine(document.getElementById("chasm-ignt-result-panel")).value;
                if (settings.config.promptPrefixMessage && settings.config.promptPrefixMessage.length > 0) {
                  messageToSend = settings.config.promptPrefixMessage + "\n" + messageToSend;
                }
                if (settings.config.promptSuffixMessage && settings.config.promptSuffixMessage.length > 0) {
                ...
                new Promise(async (accept, reject) => {
                  try {
                    ...
                    const modifier = await sender.send(settings.config.promptUserMessage);
                    if (modifier instanceof Error) {
                      statusTextNode.textContent = "상태: 오류 발생";
                      reject(Error("메시지 전송에 수정에 실패하였습니다: " + modifier.message));
                      return;
                    }
                    const modifyResult = await modifier(messageToSend);
                    if (modifyResult instanceof Error) {
                      reject(Error("유저 메시지 수정에 실패하였습니다: " + modifyResult.message));
                      return;
                    }
                    statusTextNode.textContent = "전송 완료! 페이지를 새로 고쳐 결과를 확인하세요.";
                    accept(undefined);
                  } catch (error) {
                    statusTextNode.textContent = "상태: 오류 발생";
                    reject(error);
            ...
            .addButton("chasm-ignt-delete-current", "결과 삭제", {
              onTrigger: () => {
                // @ts-ignore
                modal.__menuItems?.get("결정화 캐즘 이그나이터")?.__subMenus?.delete(result.name);
  ...
  function setupPromptLibrary(modal, panel) {
    database.prompts.toArray().then((/** @type {string | any[]} */ arr) => {
      if (arr.length <= 0) {
        ...
        for (const item of arr) {
          panel.addBoxedField(item.name, `제작자 ${item.author}, 프롬프트 길이 ${item.prompt.length}자\n` + item.description + "\n", {
            onInit: (node) => {
              node.append(
                setupClassNode("div", "chasm-ignt-svg-spaced-button", (buttonNode) => {
                  buttonNode.innerHTML = TRASH_CAN_ICON_SVG;
                  buttonNode.onclick = () => {
                    if (confirm(`정말로 프롬프트 ${item.name}을 라이브러리에서 지우시겠습니까?\n이 동작은 되돌릴 수 없습니다!`)) {
                      database.prompts
                        .where("name")
                        .anyOf(item.name)
                        .delete()
                        .then(() => {
                          modal.triggerSelect(["결정화 캐즘 이그나이터", "프롬프트 라이브러리"]);
                        });
                    }
                  };
                }),
              ...
              node.append(
                setupClassNode("div", "chasm-ignt-svg-button", (buttonNode) => {
                  ...
                  buttonNode.onclick = () => {
                    ...
                    modal.triggerSelect(["결정화 캐즘 이그나이터", "프롬프트 수정"]);
                  };
                }),
              );
            },
    ...
    panel.footer(false).addButton("chasm-ignt-load-prompt", "클립보드에서 불러오기", {
      onTrigger: (_) => {
        window.navigator.clipboard
          ...
          .then((text) => {
            ...
            if (data.description.length > 500) {
              if (!confirm(`프롬프트의 설명이 너무 깁니다. (${data.description.length}자)\n정말로 불러오시겠습니까?`)) return;
            }
            if (data.prompt.length > 10000) {
              if (!confirm(`프롬프트의 길이가 너무 깁니다. (${data.prompt.length}자)\n정말로 불러오시겠습니까?`)) return;
            }
            if (data.author.length > 30) {
              if (!confirm(`프롬프트 제작자 닉네임이 너무 깁니다. (${data.author.length}자)\n이 스크립트는 악의적, 혹은 혼란을 주기 위한 용도로 제작되었을 확률이 높습니다.\n정말로 불러오시겠습니까?`)) return;
            }
            if (data.title.length > 30) {
              if (!confirm(`프롬프트 제목이 너무 깁니다. (${data.title.length}자)\n이 스크립트는 악의적, 혹은 혼란을 주기 위한 용도로 제작되었을 확률이 높습니다.\n정말로 불러오시겠습니까?`)) return;
            }
            database.prompts
              .put({
                name: data.title,
              ...
              .then(() => {
                alert("프롬프트를 불러왔습니다.");
                ModalManager.getOrCreateManager("c2").getOpened().triggerSelect(["결정화 캐즘 이그나이터", "프롬프트 라이브러리"]);
              })
  ...
  function setupPromptModifySubmenu(panel) {
    const promptName = panel.constructInputGrid("chasm-ignt-prompt-name", "프롬프트 이름", true, {
      defaultValue: lastModified[0],
      onChange: (_, text) => {
        lastModified[0] = text;
      },
    });
    const promptAuthor = panel.constructInputGrid("chasm-ignt-prompt-author", "프롬프트 제작자", true, {
      defaultValue: lastModified[1],
      onChange: (_, text) => {
        lastModified[1] = text;
      },
    });
    const promptDescriptionContent = panel.constructTextAreaGrid("chasm-ignt-prompt-description", "프롬프트 설명", {
      defaultValue: lastModified[2],
      onChange: (node, text) => {
        lastModified[2] = text;
      },
    });
    const promptContent = panel.constructTextAreaGrid("chasm-ignt-prompt-contents", "프롬프트 내용", {
  ...
  function setupApiSettingSubmenu(panel) {
    const box = panel.constructSelectBox("설정할 API 서비스", "Gemini Api (Google)", "chasm-ignt-api-service-google", true);
    box.addGroup("API 제공자");
    box.addOption("Gemini API (Google)", "chasm-ignt-api-service-google", () => {
      for (const box of document.getElementsByClassName("chasm-ignt-api-field")) {
        box.setAttribute("chasm-ignt-hide", "true");
      }
      document.getElementById("chasm-ignt-api-key-gemini-container")?.removeAttribute("chasm-ignt-hide");
      return true;
    });
    box.addOption("Firebase Vertex AI (Google)", "chasm-ignt-api-service-firebase", () => {
      for (const box of document.getElementsByClassName("chasm-ignt-api-field")) {
        box.setAttribute("chasm-ignt-hide", "true");
      }
      document.getElementById("chasm-ignt-api-key-firebase-container")?.removeAttribute("chasm-ignt-hide");
      return true;
    });
    panel.addBoxedInputGrid("chasm-ignt-api-key-gemini", "Google Gemini API 키", "Google Gemini API 키를 입력하세요.", {
    ...
    panel.constructBoxedTextAreaGrid("chasm-ignt-api-key-gemini", "Firebase Vertex AI API 스크립트", "Firebase Vertex AI API 스크립트를 입력하세요.", {
      ...
      onChange: (_, text) => {
        ...
        if (!fetched) {
          ...
          if (!settings.config.firebaseScript || !parseVertexContent(settings.config.firebaseScript)) {
            delete settings.config.firebaseScript;
            settings.save();
          }
  ...
  function setupModuelSettings(panel) {
    panel.addSwitchBox("chasm-ignt-use-legacy-turn", "레거시 버너 턴 사용", "활성화시, 원본 캐즘의 턴 개념을 사용합니다.\n불러오는 메시지가 턴 단위로 변경되며, 턴당 2개의 메시지로 취급합니다.", {
    ...
    panel.addSwitchBox("chasm-ignt-save-modified-result", "수정된 이그나이터 결과 저장", "활성화시, 이그나이터 구동 후 나온 결과를 수정했을 때 저장합니다.\n비활성화 된 경우, 결과 탭을 다시 켜면 원본 내용으로 복구됩니다.", {
    ...
    panel.addSwitchBox("chasm-ignt-add-prompt-modify-box", "커스텀 프롬프트 수정 에이리어 추가", "활성화시, 커스텀 프롬프트가 아니더라도 다른 프롬프트 또한 즉시 수정이 가능하도록 프롬프트 창이 추가됩니다.", {
    ...
    panel.addSwitchBox("chasm-ignt-allow-result-resize", "결과 박스 크기 재조정 허용", "활성화시, 결과 박스의 크기 재조정을 허용합니다.\n결과 박스는 우측 아래를 드래그하여 크기 조정이 가능합니다.", {
    ...
    panel.addSwitchBox("chasm-ignt-custom-prompt-resize", "커스텀 프롬프트 박스 크기 재조정 허용", "활성화시, 커스텀 프롬프트 박스의 크기 재조정을 허용합니다.\n커스텀 프롬프트 박스는 우측 아래를 드래그하여 크기 조정이 가능합니다.", {
  ...
  const DEFAULT_PROMPTS = {
    fastwrtn: {
      "1:1 캐릭터": {
        description: "fastwrtn에서 사용된 스크립트입니다. 1대1 캐릭터에 초점을 맞춰 요약합니다.",
        prompt: {
          ...
          form: ["```Save-yyMMddHHmm\n", "[시간대별 정리]\n", "-yyMMddHHmm...\n", " -{contain_content}...\n", "```\n"],
          contain_content: ["상황을 최상단 작성 이후 줄바꿈", "각 캐릭터가 하나의 행동과 대화 이후 줄바꿈 후 작성", "주변의 상황", "캐릭터와 유저의 행동", "캐릭터와 유저의 대화"],
        },
      },
      시뮬레이션: {
        description: "fastwrtn에서 사용된 프롬프트입니다. 시뮬레이션 캐릭터에 초점을 맞춰 요약합니다.",
        prompt: {
    ...
    로컬AI: {
      "사건-약속-흐름 위주": {
        description: "'로컬AI' 유저가 제작한 프롬프트입니다. 사건과 약속, 흐름을 위주로 요약합니다.",
        prompt:
      ...
      "아카이브 엔진 중간축약": {
        description: "'로컬AI' 유저가 제작한 사용된 프롬프트입니다. 롤 플레잉 / 시뮬레이션에 초점이 맞춰져 있으며, 이전의 요약 사항을 포함하여 다시 요약하는 형태를 지닙니다.",
        prompt:
    ...
    Flora: {
      "감정선-서사 위주 1600자": {
        description: "'Flora' 유저가 제작한 프롬프트입니다. 감정선과 서사를 위주로 요약합니다. 프롬프트가 짧아 비교적 적은 토큰을 사용합니다.",
        prompt:
      ...
      "감정선-서사 위주 2600자": {
        description: "'Flora' 유저가 제작한 프롬프트입니다. 감정선과 서사를 위주로 요약합니다.",
        prompt:
      ...
      "구조기억 3인용 v1.1": {
        description: "'Flora' 유저가 제작한 프롬프트입니다. 3인이 등장하는 캐릭터 채팅을 목표로 합니다.",
        prompt:
      ...
      "구조기억 다인용 (4인+) v1.1": {
        description: "'Flora' 유저가 제작한 프롬프트입니다. 4인 이상이 등장하는 캐릭터 채팅을 목표로 합니다.",
        prompt:
      ...
      "구조기억 v1.1": {
        description: "'Flora' 유저가 제작한 프롬프트입니다. 최대한 높은 품질의 요약본을 목표로 합니다. 타 프롬프트보다 길이가 길어 토큰 효율이 낮습니다.",
        prompt:
      ...
      "구조기억 v3.5": {
        description: "'Flora' 유저가 제작한 프롬프트입니다. 최대한 높은 품질의 요약본을 목표로 합니다. 이전 버전보다 최적화되어 더 나은 토큰 효율을 가집니다.",
        prompt:
      ...
      "기억보조 v1.0": {
        description: "'Flora' 유저가 제작한 프롬프트입니다. 구조보다는 기억의 무결성에 초점을 맞춰 요약합니다.",
        prompt:
    ...
    니산: {
      "신디사이저 (1:1, v20251024)": {
        description: "LLM에 역할을 부여하여 요약본을 출력하도록 유도하는 특수한 프롬프트입니다. 연속된 대화에 초점이 맞춰져있으며, NPC-PC 관계를 중점으로 요약합니다. 1대1 대화에 최적화되었습니다.",
      ...
      "신디사이저 (다인용, v20251024)": {
        description: "LLM에 역할을 부여하여 요약본을 출력하도록 유도하는 특수한 프롬프트입니다. 연속된 대화에 초점이 맞춰져있으며, NPC-PC 관계를 중점으로 요약하고, NPC간의 관계에 대해서도 요약을 포함합니다. 다인 대화에 최적화되었습니다.",
  ...
  class CrackMessageSender extends PlatformMessageSender {
     ...
     *   2. modifier(summaryText) 호출 시
     *      PATCH /v3/chats/{id}/messages/{msgId} → 기존 본문 + "\n\n---\n\n" + summaryText
     *
     * @param {string} _message  (인터페이스 호환용, 미사용)
     * @returns {Promise<((summaryText: string) => Promise<void | Error>) | Error>}
     */
    async send(_message) {
      if (!this.chatRoomId) return new Error("채팅방 ID가 없습니다.");

      // Step 1: 최신 메시지 1개 조회
      const listResult = await CrackUtil.network().authFetch(
        "GET",
        `https://crack-api.wrtn.ai/crack-gen/v3/chats/${this.chatRoomId}/messages?limit=1`
      );
      if (listResult instanceof Error) return listResult;

      const messages = listResult.data?.messages ?? [];
      const latest = Array.isArray(messages) && messages.length > 0 ? messages[0] : null;
      if (!latest) return new Error("메시지 목록을 가져올 수 없습니다.");
      if (latest.role !== "assistant") return new Error("최신 메시지가 AI 응답이 아닙니다.");

      const messageId = latest._id;
      const originalContent = latest.content ?? "";

      // Step 2: modifier 반환 — 호출 시 PATCH 실행
      return async (summaryText) => {
        const newContent = originalContent + "\n\n---\n\n" + summaryText;
        const patchResult = await CrackUtil.network().authFetch(
          "PATCH",
          `https://crack-api.wrtn.ai/crack-gen/v3/chats/${this.chatRoomId}/messages/${messageId}`,
          { message: newContent }
        );
        if (patchResult instanceof Error) return patchResult;
      };
    }
  }
  ...
  class CrackMessageFetcher extends PlatformMessageFetcher {
    ...
    async fetch(maxCount) {
      ...
      return items.map((it) => new PlatformMessage(it.role, it.role, it.content));
    }
  ...
  class CrackUserNoteUtility extends UserNoteUtility {
    ...
    constructor(chatId) {
      super();
      this.chatId = chatId;
      this.expectedUrl = CrackUtil.path().isCharacterPath() ? `https://contents-api.wrtn.ai/character-chat/single-character-chats/${chatId}` : `https://crack-api.wrtn.ai/crack-gen/v3/chats/${chatId}`;
    }
     ...
     */
    async fetch() {
      const result = await CrackUtil.network().authFetch("GET", this.expectedUrl);
  ...
  class CrackPersonaUtility extends PlatformPersonaUtility {
     ...
     */
    async getRepresentivePersona() {
      const userIdFetch = await CrackUtil.network().authFetch("GET", "https://crack-api.wrtn.ai/crack-api/profiles");
      ...
      const personaFetch = await CrackUtil.network().authFetch("GET", `https://crack-api.wrtn.ai/crack-api/profiles/${userId}/chat-profiles`);
      ...
      const roomData = await CrackUtil.network().authFetch("GET", `https://contents-api.wrtn.ai/character-chat/v3/chats/${this.chatId}`);
      if (!roomData) {
  ...
  async function injectInputbutton() {
    ...
    if (top) {
      ...
      const buttonCloned = GenericUtil.cloneCast(expectedTop.childNodes[0]);
  ...
  "loading" === document.readyState ? (document.addEventListener("DOMContentLoaded", doInjection), window.addEventListener("load", doInjection)) : "interactive" === document.readyState || "complete" === document.readyState ? await doInjection() : setTimeout(doInjection, 100);
})();